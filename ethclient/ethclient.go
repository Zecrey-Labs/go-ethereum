...
func (ec *Client) BlockReceipts(ctx context.Context, hashOrNumber rpc.BlockNumberOrHash) (types.Receipts, error) {
...
func (ec *Client) BlocksWithTxsAndReceipts(ctx context.Context, blockNums []rpc.BlockNumber) ([]*types.BlockWithTxsAndReceipts, error) {
	...
	for _, blockNum := range blockNums {
		blockWithTxs, err := ec.BlockByNumber(ctx, big.NewInt(blockNum.Int64()))
		if err != nil {
			return nil, err
		}
		receipts, err := ec.BlockReceipts(ctx, rpc.BlockNumberOrHash{BlockNumber: &blockNum})
		if err != nil {
			return nil, err
		}
		blocks = append(blocks, &types.BlockWithTxsAndReceipts{
			Block:    blockWithTxs,
			Receipts: receipts,
		})
	}
...
func (ec *Client) SendTransactionWithFrom(ctx context.Context, from common.Address, tx *types.Transaction) error {
	data, err := tx.MarshalBinary()
	if err != nil {
		return err
	}
	return ec.c.CallContext(ctx, nil, "eth_sendRawTransactionWithFrom", from, hexutil.Encode(data))
}
